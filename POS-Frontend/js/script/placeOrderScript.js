        ...
        itemList.forEach(function (item) {
            if(value == "clicked"){
                if (item.code == $("#selectItemCode").val()){
                    ...
                    $("#txtitemQty").text(item.qtyOnHand);
                }
            }else if (value == "onload") {
...
$("#save-order").click(function () {
    ...
    console.log(custid)
    order = {orderId:orderid,orderDate:orderdate,customerId:custid,orderDetails:orderdetail};
    ...
    $.ajax(postAjaxConfig).done(function (response, textStatus, jqxhr) {
        if (jqxhr.status === 201) {
            alert(response + " : Order has been successfully added");
...
function generateOrderID() {
    ...
    $.ajax(ajaxGetConfig).done(function (orderList,textStatus,iqxhr) {
        ...
        $("#orderID").text(2+1);
        // $("#orderID").text((parseInt(orderid)+1));
    }).fail(function (jqxhr, textStatus, errorMsg) {
        console.log(errorMsg);
    });
}
